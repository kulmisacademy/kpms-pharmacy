-- Pharmacy `manager` role: JSON-driven access like staff, with separate app defaults.
-- Invitations + profile updates must accept `manager`.
...
begin
  ...
  if coalesce(trim(p_role), '') not in ('staff', 'cashier', 'pharmacist', 'manager') then
...
begin
  ...
  if v_role is not null and v_role <> '' then
    if v_role not in ('staff', 'cashier', 'pharmacist', 'pharmacy_owner', 'manager') then
      raise exception 'Invalid role';
    end if;
